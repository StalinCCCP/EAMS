<template>
    <n-space vertical :size="12">
      ...
      <n-form-item>
       ...
       <n-form-item>
        ...
        <n-form-item>
          <n-input size="large" v-model:value="req.SoftwareName" placeholder="Find..." />
        </n-form-item>
        <n-form-item>
  ...
  import createColumns from '@/models/SoftwareMaintenanceTable'
  import {ref} from 'vue'
...
//   const Location=ref()
  ...
  const create = ()=>{
      router.push('/createsoftwaremaintenance')
  }
  const post = () => {
    http
      .post("/p/user/smlq", req.value, {
        validateStatus: function (status) {
          return true;
        },
      })
...
//   }
  ...
  const columns=createColumns({click(row){
      ...
      http
      .delete("/p/admin/smdlt", {data:{MaintenanceProcessID:row.MaintenanceProcessID}}, {
        validateStatus: function (status) {
          return true;
        },
      })
