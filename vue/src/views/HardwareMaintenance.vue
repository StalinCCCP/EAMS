<template>
  ...
  import createColumns from '@/models/HardwareMaintenanceTable'
  import {ref} from 'vue'
...
//   const Location=ref()
  ...
  const create = ()=>{
      router.push('/createhardwaremaintenance')
  }
  const post = () => {
    http
      .post("/p/user/hmlq", req.value, {
        validateStatus: function (status) {
          return true;
        },
      })
...
//   }
  ...
  const columns=createColumns({click(row){
      ...
      http
      .delete("/p/admin/hmdlt", {data:{MaintenanceProcessID:row.MaintenanceProcessID}}, {
        validateStatus: function (status) {
          return true;
        },
      })
